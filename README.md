...
9.`攻击小鬼与饿鬼给肉山造成真伤`只在FTW和天顶种子生效，防止多人模式下近战职业无法更好输出作出的优化

10.`自定义转移伤害表`所有转移伤害都视为真实伤害，`输出怪物`为BOSS，`输入怪物`为小怪，`停转生命`则是生命到达多少不再转移，`涵盖暴击`则是包含所有对小怪的暴击伤害转移

11.`转移伤害`只有在击中一次BOSS才会生效
