...
public static class Tool
{
    ...
    public static void WriteName()
    {
        foreach (var list in DamageRuleLoot.Config.TList)
        {
            ...
            {
                string npcName = (string)Lang.GetNPCName(list.NPCA);
                list.Name = npcName;
            }
        }
    }
    ...
    public static bool CombDmg(NpcKilledEventArgs args, int i, bool flag, int id, int[] ids, float value)
    {
        ...
        if (index >= 0)
        {
            StrikeNPC? st = StrikeNPC.strikeNPC.Find(x => x.npcID == id);
            if (st == null)
            {
    ...
    public static void CombDmg3(int i, ref StrikeNPC? strike2, int id, int[] ids, float value)
    {
        ...
        {
            if (!DamageRuleLoot.Config.Prime) return; //比CombDmg2多了这一行 这个Config.Prime同时也影响伤害转移
    ...
    public static void TransformDamage(NPC npc, int Damage, Player plr, Configuration.ItemData Custom, StrikeNPC? strike)
    {
        ...
        if (DamageRuleLoot.Config.TransferInfo)
        {
            ...
            {
                TShock.Utils.Broadcast($"[c/FBF069:【转伤】] 玩家:[c/F06576:{plr.name}] " +
                    $"攻击对象:[c/AEA3E4:{npc.FullName}] | " +
                    $"转移:[c/6DDA6D:{strike.npcName}] 伤害:[c/F06576:{Damage}] " +
                    $"生命:[c/FBF069:{Main.npc[strike.npcIndex].life}]", 202, 221, 222);
            }
